...
export default {
  ...
  methods: {
    async getContact() {
      const resData = await axios.get("http://127.0.0.1:8003/api/contact/");
    ...
    async insertContact() {
      ...
      await axios.post("http://127.0.0.1:8003/api/contact/", sendData);
    ...
    async updateContact(id, name, email) {
      ...
      await axios.put("http://127.0.0.1:8003/api/contact/" + id, sendData);
      await this.getContact();
    },
    async deleteContact(id) {
      await axios.delete("http://127.0.0.1:8003/api/contact/" + id);
      await this.getContact();
    },
  },
  created() {
